...
exports.messageHydration = {
    ...
    functions: {
        ...
        webhook: (message, ctx) => message.webhook
            ? new index_js_1.MessageWebhook(ctx, message.webhook, message.author)
            : undefined,
        content: (message) => message.content,
        systemMessage: (message, ctx) => index_js_1.SystemMessage.from(ctx, message.system),
        attachments: (message, ctx) => message.attachments.map((file) => new index_js_1.File(ctx, file)),
        editedAt: (message) => new Date(message.edited),
        embeds: (message, ctx) => message.embeds.map((embed) => index_js_1.MessageEmbed.from(ctx, embed)),
